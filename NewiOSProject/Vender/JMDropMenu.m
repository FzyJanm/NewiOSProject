...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(JMDropMenuType)type LayoutType:(JMDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate radius:(CGFloat)raddius{
    if (self = [super initWithFrame:frame]) {
        ...
        _numberOfRow = titleArr.count;
        
        //类型判断
        if (type == JMDropMenuTypeWeChat || type == JMDropMenuTypeWeChatNoneArrow) {
...
+ (instancetype)showDropMenuFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(JMDropMenuType)type LayoutType:(JMDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate radius:(CGFloat)raddius{
    return [[self alloc] initWithFrame:frame ArrowOffset:arrowOffset TitleArr:titleArr ImageArr:imageArr Type:type LayoutType:layoutType RowHeight:rowHeight Delegate:delegate radius:raddius];
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.separatorColor = LiveSepartorColor();
        if (_numberOfRow*_rowHeight+8<=self.frame.size.height) {
        ...
        _tableView.layer.cornerRadius = _radius;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleL.textAlignment = self.alignment;
    return cell;
}
...
- (UIView *)cover {
    if (!_cover) {
        ...
        _cover.backgroundColor = [UIColor colorWithWhite:1 alpha:0];
        _cover.tag = 100;
    }
    return _cover;
}
...
#pragma mark - 蒙版点击
- (void)tapCoverClick {
    [self.delegate coverViewDidClick];
    [self removeDropMenu];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat alpha = 0.0;
    if (_type == JMDropMenuTypeQQNoneArrow || _type==JMDropMenuTypeWeChatNoneArrow) {
    }else {
        alpha = 1;
    }
    ...
    if (_type == JMDropMenuTypeQQNoneArrow || _type==JMDropMenuTypeWeChatNoneArrow) {
        ...
        aColor = [UIColor colorWithRed:r green:g blue:b alpha:alpha];
    }
...
- (void)layoutSubviews {
    ...
    self.imageIV.frame = CGRectMake(10, self.screenCenter - 8, 20, 20);
    if (self.model.layoutType == JMDropMenuLayoutTypeTitle) {
        self.titleL.frame = CGRectMake(0,0, self.frame.size.width, self.frame.size.height);
    } else {
