...
void vcodeBtnConfig(UIButton *verifyBtn,NSString *title){
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                verifyBtn.backgroundColor = [UIColor colorWithRed:1.f green:234.f/255.f blue:248.f/255.f alpha:1.f];
            });
        }else{
            //            int minutes = timeout / 60;
            int seconds = timeout % 60;
            seconds = timeout==60?60:seconds;
            NSString *strTime = [NSString stringWithFormat:@"%.2d", seconds];
            dispatch_async(dispatch_get_main_queue(), ^{
                NSLog(@"____%@",strTime);
                [verifyBtn setTitle:[NSString stringWithFormat:@"%@秒后重新发送",strTime] forState:UIControlStateNormal];
                verifyBtn.enabled = NO;
                verifyBtn.backgroundColor = [UIColor colorWithRed:(float)218.f/255.f green:(float)218.f/255.f blue:(float)218.f/255.f alpha:1.0f];
