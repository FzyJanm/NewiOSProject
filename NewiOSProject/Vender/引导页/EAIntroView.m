...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page == pageViews.count  && self.swipeToExit) {
    ...
    //NSLog(@"个数 %d %d",page,pageViews.count);
...
- (void)showPanelAtPageControl {
    ...
    [self.scrollView setContentOffset:CGPointMake(self.currentPageIndex * APPW, 0) animated:YES];
}

- (void)skipIntroduction {
    if (self.scrollerViewBlock) {
        self.scrollerViewBlock();
    }
