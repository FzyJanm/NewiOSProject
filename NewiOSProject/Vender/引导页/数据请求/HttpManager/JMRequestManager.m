...
// post
- (void)POST:(NSString *)urlString parameters:(id)parameters completion:(void (^)(JMBaseResponse *))completion
{
    [self request:@"POST" URL:urlString parameters:parameters completion:completion];
}

//get
- (void)GET:(NSString *)urlString parameters:(id)parameters completion:(void (^)(JMBaseResponse *))completion
{
    [self request:@"GET" URL:urlString parameters:parameters completion:completion];
}

#pragma mark - post & get
- (void)request:(NSString *)method URL:(NSString *)urlString parameters:(id)parameters completion:(void (^)(JMBaseResponse *response))completion
{
    [self logRequest:urlString params:parameters];
    if (self.isLocal) {
...
//    }
    ...
    void(^success)(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) = ^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSString *desc = [NSString stringWithFormat:@"%@", dataDict[@"desc"]];
        if ([code isEqualToString:@"2"]) {
            [JMProjectManager sharedJMProjectManager].loginUser = nil;
//            [[UIWindow jm_currentNavigationController]popToRootViewControllerAnimated:NO];
...
#pragma mark - 加载本地数据
static NSString *jsonFileDirectory = @"JMLocalJsons";
- (void)requestLocal:(NSString *)urlString completion:(void (^)(JMBaseResponse *response))completion
{
...
#pragma mark - 处理数据
- (void)wrapperTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error completion:(void (^)(JMBaseResponse *response))completion
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        
        JMBaseResponse *response = [self convertTask:task responseObject:responseObject error:error];
...
#pragma mark - 包装返回的数据
- (JMBaseResponse *)convertTask:(NSURLSessionDataTask *)task responseObject:(id)responseObject error:(NSError *)error
{
    
    JMBaseResponse *response = [JMBaseResponse new];
    
    if (!JMIsEmpty(responseObject)) {
...
#pragma mark - 打印返回日志
- (void)LogResponse:(NSString *)urlString response:(JMBaseResponse *)response
{
    NSLog(@"\n[%@]---%@\n", urlString, response);
}

-(void)logRequest:(NSString *)urlString params:(NSDictionary *)params{
    NSLog(@"\n[%@]---%@\n",urlString,params);
}
...
- (void)upload:(NSString *)urlString parameters:(id)parameters formDataBlock:(NSDictionary<NSData *, JMDataName *> *(^)(id<AFMultipartFormData> formData, NSMutableDictionary<NSData *, JMDataName *> *needFillDataDict))formDataBlock progress:(void (^)(NSProgress *progress))progress completion:(void (^)(JMBaseResponse *response))completion
{
    ...
    [self POST:urlString parameters:parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [JMProjectManager sharedJMProjectManager].loginUser;
        if (datas) {
            [datas enumerateKeysAndObjectsUsingBlock:^( NSData * _Nonnull data, JMDataName * _Nonnull name, BOOL * _Nonnull stop) {
                NSArray *array = [name componentsSeparatedByString:@"_"];
                NSString *fileName = [array lastObject];
                NSString *keyName = [array firstObject];
                if(fileName == nil){
                    fileName = @"random";
                }
                [formData appendPartWithFileData:data name:keyName fileName:fileName mimeType:mineType];
            }];
        }
...
- (void)configSettings
{
    ...
    //自定义处理数据
    self.responseFormat = ^JMBaseResponse *(JMBaseResponse *response) {
        if(!response.error){
            NSMutableDictionary *result = response.responseObject;
            NSString *code = [result getJsonValue:@"code"];
            if(code.intValue != 1){
                response.error = [NSError errorWithDomain:NSCocoaErrorDomain code:code.intValue userInfo:nil];
                response.errorMsg = [result getJsonValue:@"desc"];
                if(code.intValue == -2){
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[JMProjectManager sharedJMProjectManager] showLoginViewController];
                    });
...
+ (instancetype)manager {
    JMRequestManager *manager = [super manager];
    manager.requestSerializer.timeoutInterval = 10*60;
    [manager configSettings];
    return manager;
}

static JMRequestManager *_instance = nil;
