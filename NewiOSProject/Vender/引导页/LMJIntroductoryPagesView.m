...
- (void)loadPageView
{
    ...
    self.scrollView.contentSize = CGSizeMake((self.imagesArray.count + 1) * APPW, APPH);
    ...
    [self.imagesArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        imageView.frame = CGRectMake(idx * APPW, 0, APPW, APPH);
...
{
    if (scrollView.contentOffset.x >= (_imagesArray.count) * APPW) {
        [self removeFromSuperview];
    }
}
...
- (UIPageControl *)pageControl
{
    if(!_pageControl)
    {
        UIPageControl *pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(APPW/2, APPH - 60, 0, 40)];
        pageControl.backgroundColor = [UIColor colorWithWhite:.2 alpha:.5];
        pageControl.pageIndicatorTintColor = RGBA(49, 49, 49, .5);
        pageControl.currentPageIndicatorTintColor = FZYColor(244, 244,244 );
