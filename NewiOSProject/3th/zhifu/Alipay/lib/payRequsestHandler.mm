...
// 负责人：李启波（marcyli）
//============================================================
- ( NSMutableDictionary *)sendPayByOrderTitle:(NSString *)Title Price:(NSString *)price OrderID:(NSString *)orderID UUID:(NSString *)uuid
{
    //订单标题，展示给用户
 ...
 //   orderno=orderID?orderID:orderno;
    NSMutableDictionary *packageParams = [NSMutableDictionary dictionary];
    
    [packageParams setObject: appid             forKey:@"appid"];       //开放平台appid
    [packageParams setObject: mchid             forKey:@"mch_id"];      //商户号
    [packageParams setObject: uuid              forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户9oooooo
    [packageParams setObject: [NSString stringWithFormat:@"%@",WXNotifiURL]   forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: orderID           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"192.168.1.1"    forKey:@"spbill_create_ip"];//116.255.251.77//发器支付的机器ip
